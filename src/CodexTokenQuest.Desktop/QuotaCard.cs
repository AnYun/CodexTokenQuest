...
internal sealed class QuotaCard : Control
{
    ...
    internal void UpdateCountdown(DateTimeOffset now)
    {
        ...
        else
        {
            ...
            _countdown = remaining <= TimeSpan.Zero
                ? $"RESET {local:MM/dd HH:mm} // SYNCING"
                : $"RESET {local:MM/dd HH:mm} // {FormatDuration(remaining)}";
        }
        Invalidate();
    }
    ...
    protected override void OnPaint(PaintEventArgs eventArgs)
    {
        ...
        TextRenderer.DrawText(graphics, $"STA {remaining:0.#}", valueFont, new Rectangle(230, 7, 105, 16), accent, TextFormatFlags.Right | TextFormatFlags.NoPadding);
